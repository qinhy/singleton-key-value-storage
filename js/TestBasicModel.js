import { BasicStore, Model4Basic } from "./BasicModel.js"
    ...
    console.assert(
      objr !== null &&
        JSON.stringify(obj.model_dump_json_dict()) ===
          JSON.stringify(objr.model_dump_json_dict()),
  ...
  test_group() {
    ...
    console.assert(
      JSON.stringify(
        group
          .get_controller()
          .get_child(group.children_id[0])
          .model_dump_json_dict()
      ) === JSON.stringify(obj.model_dump_json_dict()),
      "The retrieved value should match the child value."
    )

    const group2_id = this.store
      .add_new_group(new Model4Basic.AbstractGroup())
      .get_id()
